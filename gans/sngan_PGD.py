...
def adverse(model, x, alpha=2/255 * 2, eps=8/255 * 2, M=7):
    # PGD start
    ...
    for _ in range(M):
        ...
        loss = model(x_adv)
        grad = torch.autograd.grad(loss, delta, torch.ones_like(loss))[0]

        with torch.no_grad():
            delta = delta + alpha * grad.sign()
            delta = torch.clamp(delta, min=-eps, max=eps).detach_()
    # PGD end
    ...
    with trange(1, FLAGS.total_steps + 1, dynamic_ncols=True) as pbar:
        for step in pbar:
            ...
            for _ in range(FLAGS.n_dis):
                ...
                net_D_real = net_D(adverse(net_D, real))
